...
class KompasSpider(scrapy.Spider):
    ...
    start_urls = [
        'https://www.kompas.com/terkini',  # Latest news
        'https://nasional.kompas.com/',     # National
        'https://megapolitan.kompas.com/',  # Metropolitan
    ]
    ...
    def parse(self, response):
        ...
        # Find article links (Kompas uses various patterns)
        article_links = response.css('a.article__link::attr(href)').getall()

        if not article_links:
            # Try alternative selectors
            article_links = response.css('div.latest__item a::attr(href)').getall()

        if not article_links:
            article_links = response.css('div.article__list a::attr(href)').getall()
    ...
    def parse_article(self, response):
        ...
        try:
            ...
            # Extract content
            content_paragraphs = response.css('div.read__content p::text').getall()
            ...
            # Extract author
            author = response.css('div.read__author__name::text').get()
            if not author:
                author = response.css('div.author__name::text').get()

            # Extract published date
            published_at = response.css('div.read__time::attr(data-publish)').get()
            if not published_at:
